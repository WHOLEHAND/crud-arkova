...
$sqlUsers = "SELECT id, user_name, names, last_names, identity_card, email, 
                    ...
                    END AS role
             FROM users
             WHERE state_user = 0"; // Solo selecciona usuarios activos
...
<body>
    ...
    <div class="container py-3">
        ...
        <div class="row justify-content-center mt-4">
            ...
            <div class="col-auto">
                <button id="confirmDisableUserMulti" class="btn btn-secondary"><i class="fa-solid fa-user-check"></i> Habilitar Usuarios</button>
                <form id="disableUserMultiForm" action="disableUserMultiModal.php" method="post" style="display: none;">
                    <input type="hidden" name="selected_ids" id="selectedIds">
                </form>
            </div>
            <div class="col-auto">
                <a href="/crud-arkova/app/pdf reports/disabled_users_PDF.php" target="_blank" class="btn btn-outline-danger" title="Generar Informe PDF"><i class="fa-solid fa-file-pdf"></i> PDF</a>
            </div>
            <div class="col-auto">
                <a href="/crud-arkova/app/excel reports/disabled_users_excel.php" target="_blank" class="btn btn-outline-success" title="Generar Informe Excel"><i class="fa-solid fa-file-excel"></i> Excel</a>
            </div>
        </div>
        <br>
                ...
                <?php while ($row_users = $users->fetch_assoc()) { ?>
                    <tr>
                        ...
                        <td class="align-middle">
                            ...
                            <a href="#" class="btn btn-sm btn-secondary openMove" data-bs-toggle="modal" data-bs-target="#enableUserModal" title="Habilitar Usuario" data-bs-id="<?= $row_users['id']; ?>"><i class="fa-solid fa-user-check"></i></a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
    ...
    <?php require 'enableUserModal.php'; ?>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Activar usuario
            // Al hacer clic en el botón de habilitar
            let enableUserModal = document.getElementById('enableUserModal');
            enableUserModal.addEventListener('shown.bs.modal', event => {
                let button = event.relatedTarget;
                let id = button.getAttribute('data-bs-id');
                enableUserModal.querySelector('.modal-footer #id').value = id;
            });
        });
        ...
        // Modal Habilitar Multiples Usuarios
        document.addEventListener('DOMContentLoaded', () => {
            document.getElementById('confirmDisableUserMulti').addEventListener('click', () => {
                ...
                const confirmAction = confirm("¿Estás seguro de que deseas habilitar los usuarios seleccionados?");
                if (confirmAction) {
        ...
        // JavaScript para mostrar las alertas con los mensajes del modal de habilitar usuarios
        document.getElementById("enableUserForm").addEventListener("submit", async function (event) {
            event.preventDefault(); // Evitar que el formulario se envíe de forma predeterminada
            ...
            if (result.error) {
                ...
                if (result.redirect) {
                    ...
                    // Si se debe recargar la página
                    setTimeout(() => {
                        const modal = bootstrap.Modal.getInstance(document.getElementById('enableUserModal'));
                        if (modal) {
                            modal.hide();
                        }
