...
// Asegúrate de que estás utilizando la sesión activa para determinar así el nivel de usuario
$user_level = $_SESSION['level_user'] ?? null;

// Modificar la consulta SQL según el nivel de usuario
if ($user_level == 0) { // Administrador
...
<html lang="es">
<head>
    <title>REPORTES OBSOLETOS</title>
    <!-- Required meta tags -->
    <meta charset="UTF-8">
    ...
    $(document).ready(function() {
        const table = $('#tbl_reports').DataTable();

        // Obtener el nivel de usuario de PHP
        const userLevel = '<?php echo $user_level; ?>';
        if (userLevel !== '0') { // Si no es administrador
            const columnsToHide = [0, 6]; // Índices de las columnas a ocultar para usuarios normales (Checkbox, Acción)
            columnsToHide.forEach(index => {
                table.column(index).visible(false);
            });
            table.draw(); // Actualiza la tabla después de ocultar columnas
        }
    });
    </script>
    ...
    <style>
        table {
            width: 100%;
            table-layout: auto; /* Cambia a "fixed" si quieres una distribución uniforme */
        }
        .admin-only {
            display: none;
        }
...
<body>
    ...
    <div class="container py-3">
      <div>
        <h2 class="text-center">Reportes Obsoletos</h2>
        ...
        <div class="row justify-content-center mt-4">
            ...
            <div class="col-auto">
                <button id="confirmMoveMulti" class="btn btn-secondary"><i class="fa-solid fa-recycle"></i> Restaurar Reportes</button>
            ...
            <?php } ?>
            <div class="col-auto">
                <a href="/crud-arkova/app/pdf reports/obsolete_reports_PDF.php" target="_blank" class="btn btn-outline-danger"><i class="fa-solid fa-file-pdf"></i> Informe PDF</a>
            </div>
            <div class="col-auto">
                <a href="/crud-arkova/app/excel reports/obsolete_reports_excel.php" target="_blank" class="btn btn-outline-success"><i class="fa-solid fa-file-excel"></i> Informe Excel</a>
            </div>
        </div>
        <hr>
        ...
        >
          ...
          <tbody>
                <?php while ($row_report = $reports->fetch_assoc()) { ?>
                    <tr>
                        <td><input class="form-check-input admin-only" type="checkbox" name="selected_reports[]" value="<?php echo $row_report['id']; ?>"></td>
                        ...
                        <td class="align-middle admin-only action-column">
                            ...
                            <a href="#" class="btn btn-sm btn-secondary openMove" data-bs-toggle="modal" data-bs-target="#restoreModal" title="Restaurar Reporte" data-bs-id="<?= $row_report['id']; ?>"><i class="fa-solid fa-recycle"></i></a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
    ...
    <script>
        ...
        viewImgModal.addEventListener('shown.bs.modal', event => {
            ...
            .then(response => response.json())
            .then(data => {
                imgReport.src = '<?= $dir ?>/' + data.id + '.jpg'; // Actualizar la fuente de la imagen
            })
            .catch(err => console.log(err));
        });
        ...
        restoreModal.addEventListener('shown.bs.modal', event => {
            let button = event.relatedTarget;
            let id = button.getAttribute('data-bs-id');
            restoreModal.querySelector('.modal-footer #id').value = id;
        });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.getElementById('confirmMoveMulti').addEventListener('click', () => {
                ...
                const confirmAction = confirm("¿Estás seguro que deseas Restaurar los reportes seleccionados?");
                if (confirmAction) {
        ...
        // Script de gestión de contenido solo para el admin
